...
export default function App({ url, position}) {
    const { RSX, RSY, RSZ, RSRotation, RSVolume, RSRolloff } = useControls({
        RSX: {
          value: position[0],
          min: -20,
          max: 20,
  ...
  return (
      ...
      <Suspense fallback={null}>
        <mesh position={[RSX, RSY, RSZ]} rotation={[0, RSRotation, 0]}>
        <sphereGeometry args={[0.1, 8, 8]} />
          <meshStandardMaterial color="hotpink" wireframe />
          <Sound url={url} volume={RSVolume}  rolloffFactor={RSRolloff} />
        </mesh>
      </Suspense>
