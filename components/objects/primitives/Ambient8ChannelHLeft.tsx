...
export default function App({ url, position }) {
  const { HLX, HLY, HLZ, HLRotation, HLVolume, HLRolloff } = useControls({
    HLX: {
      value: position[0],
      min: -20,
      max: 20,
  ...
  return (
    <Suspense fallback={null}>
      <mesh position={[HLX, HLY, HLZ]} rotation={[0, HLRotation, 0]}>
        <sphereGeometry args={[0.1, 8, 8]} />
        <meshStandardMaterial color='hotpink' wireframe />
        <Sound url={url} volume={HLVolume} rolloffFactor={HLRolloff} />
      </mesh>
    </Suspense>
  )
}
